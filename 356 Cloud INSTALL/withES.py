...
def addQuestion():
	...
	if(request.method == 'POST'):
		...
		#####################
		es.index(index='question', doc_type="place",id=pid, body= question)
		#####################
...
def getQuestion(IDD):
	...
	if request.method == 'GET':
		...
		#####################
		es.delete(index='question', doc_type="place",id=IDD)
		es.index(index='question', doc_type="place",id=IDD, body= question['question'])
		#####################
	...
	if request.method == 'POST':
		...
		if upvote == True:
			if result == None:
				...
				updateQuestionScore(pid, realUser, 1, 1, IDD)
			elif result['vote'] ==  1:
				...
				upvoteTable.update_one({'username':user, 'pid': pid} , { "$set": {'vote': 1} } )
				updateQuestionScore(pid, realUser, 2, 1, IDD)
		#################################################FALSE##########################################
		elif upvote == False:
			if result == None:
				upvoteTable.insert({'username': user, 'pid': pid, 'vote': -1})
...
def acceptAnswer(IDD):
	if request.method == 'POST':
		...
		####################
		useElastic = questionTable.find_one({'_id': pid })
		es.delete(index='question', doc_type="place",id=IDD)
		es.index(index='question', doc_type="place",id=IDD, body= useElastic)
		######################
	return responseOK({'status': 'OK'})
...
def search():
	if request.method == 'POST':
		...
		if len(query) == 0:
			answer = filter_without_query(timestamp, limit, sort_by, tags, has_media, accepted)
...
def updateQuestionScore(pid, user, qval, uval, IDD):
	...
	##############
	useElastic = questionTable.find_one(  {'_id': ObjectId(pid)} )
	es.delete(index='question', doc_type="place",id=IDD)
	es.index(index='question', doc_type="place",id=IDD, body= useElastic)
	##############
